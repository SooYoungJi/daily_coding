...
def solution(k, score):
    ...
    wall_of_fame = []
    for i in score:
        wall_of_fame.append(i)
        wall_of_fame.sort(reverse=True)
        if len(wall_of_fame) < k:
            answer.append(wall_of_fame[-1])
        else:
            answer.append(wall_of_fame[k-1])
    return answer
...
'''
k보다 클 때 가장 작은 수를 빼버리는 방법!
그리고 min을 answer에 append하는 방법!
'''